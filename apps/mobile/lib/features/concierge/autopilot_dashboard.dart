...
class _AutoPilotDashboardState extends State<AutoPilotDashboard> {
  ...
  Future<void> _processRequest(Map<String, dynamic> request) async {
    ...
    await _speakAndLog("Processing request for ${request['schoolName']}, ${request['className']}, Subject ${request['subject']}.");

    // Step 2: Generation details
    await _speakAndLog("Teacher ${request['teacherName']} asked for questions from ${request['topic']}. I have generated 10 MCQs.");
    ...
    await _speakAndLog("Assigning this to ${students[0]}, ${students[1]}, and others. Mr. Staff, shall I publish?");
  ...
  void _listenForConfirmation() async {
    ...
    if (available) {
      ...
      _speech.listen(onResult: (val) {
        ...
        setState(() => _consoleLogs.add("[Mic unavailable, waiting for manual click]"));
    }
  }
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                    const Text("LIVE CONSOLE", style: TextStyle(color: Colors.green, fontFamily: 'Courier', fontWeight: FontWeight.bold)),
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: _currentRequest == null
                ? const Center(child: Text("Scanning for requests...", style: TextStyle(fontSize: 18, color: Colors.grey)))
                : Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                        const Icon(Icons.school, size: 64, color: Colors.blue),
                        const SizedBox(height: 16),
                        Text("${_currentRequest!['schoolName']}", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                        Text("${_currentRequest!['className']} - ${_currentRequest!['subject']}", style: const TextStyle(fontSize: 18)),
                        ...
                        Card(
                            ...
                            child: Padding(
                                ...
                                child: Column(
                                    children: [
                                        const Text("Quiz Preview", style: TextStyle(fontWeight: FontWeight.bold)),
                                        const Divider(),
                        ...
                        ElevatedButton(
                            ...
                            child: const Text("Yes, Send it (Manual)", style: TextStyle(fontSize: 18)),
                        )
                    ],
                  ),
            ),
